...
function Invoke-DownloadWithProgress {
    ...
    # Enable every TLS version available on this machine
    try {
    ...
    function _showProgress($totalRead, $total, $lastRead, $lastTime, $startTime) {
        $now = [DateTime]::Now
        if (($now - $lastTime).TotalSeconds -lt 0.4) { return $lastRead, $lastTime }
        ...
        return $totalRead, $now
    }

    # --- Method 1: HttpClient chunked stream (PS4+, .NET 4.5+) ---
    try {
        ...
        while (($read = $inStream.Read($buf, 0, $buf.Length)) -gt 0) {
            ...
            $lastRead, $lastTime = _showProgress $totalRead $total $lastRead $lastTime $startTime
        }
    ...
    # Spinner helper used by fallback methods
    ...
    # --- Method 2: WebClient (all .NET versions) ---
    try {
        ...
        _spinDownload $job $Dest $Label
        Receive-Job $job -ErrorAction Stop | Out-Null
        Remove-Job $job -Force -ErrorAction SilentlyContinue
        if (Test-Path $Dest) { return $true }
        throw "File not found after download"
    } catch { Write-Host "`r  Method 2 (WebClient) failed — trying Invoke-WebRequest..." -ForegroundColor DarkGray }
    ...
    try {
        ...
        if (Test-Path $Dest) { return $true }
        throw "File not found after download"
    } catch { Write-Host "`r  Method 3 (Invoke-WebRequest) failed — trying BITS...   " -ForegroundColor DarkGray }
    ...
    } catch { Write-Host "`r  Method 4 (BITS) failed — trying certutil...            " -ForegroundColor DarkGray }
