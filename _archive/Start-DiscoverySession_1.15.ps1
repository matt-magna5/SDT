<#
.SYNOPSIS
    Magna5 Server Discovery - Session Launcher v1.15
...
$invokeScriptName = 'Invoke-ServerDiscovery_1.15.ps1'
...
foreach ($t in $allVMTargets) {
    ...
    $clr = if ($t._Offline) { 'DarkGray' } else { 'White' }
    Write-Host ("  {0,-30} {1,-20} {2,-12} {3}" -f $t.Name, $ipsShort, $srcShort, $pwr) -ForegroundColor $clr
}
Write-Host ""
...
if ($outputFiles.Count -gt 0) {
    ...
    Write-Host "          (Include any -inventory- JSON files in the same message)" -ForegroundColor DarkGray
}
