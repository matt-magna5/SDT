...
# Serialize to JSON - try progressively lower depth if high depth fails
# ConvertTo-Json can fail at depth 20 with circular refs or complex WMI objects
$json = $null
$jsonErr = ''
if ($PSMaj -ge 3) {
    foreach ($depth in @(20, 10, 6)) {
        try {
            $json = $discoveryResult | ConvertTo-Json -Depth $depth -ErrorAction Stop
    ...
    try {
        ...
        $json = $serializer.Serialize($discoveryResult)
