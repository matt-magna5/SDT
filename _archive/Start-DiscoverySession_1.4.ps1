...
# Validates syntax of Invoke-ServerDiscovery before running anything.
# If the local copy is corrupt (common with RDP/SMB file transfers truncating
# large PS1 files), automatically falls back to C:\Temp\M5Discovery\.
...
$invokeScriptName = 'Invoke-ServerDiscovery_1.4.ps1'
$DiscoveryScript  = Join-Path $PSScriptRoot $invokeScriptName

if (Test-ScriptIntegrity $DiscoveryScript) {
    Write-Host "  (^_^)  Companion script OK: $DiscoveryScript" -ForegroundColor DarkGreen
} else {
    $localCopy = (Test-Path $DiscoveryScript)
    $sz = if ($localCopy) { (Get-Item $DiscoveryScript).Length } else { 0 }
    Write-Host ""
    Write-Host "  (>_<)  $invokeScriptName in script folder is missing or corrupt (${sz} bytes)." -ForegroundColor Yellow
    Write-Host "         This usually means the file was truncated during transfer (RDP/SMB)." -ForegroundColor DarkGray
    Write-Host "         Checking fallback location C:\Temp\M5Discovery\..." -ForegroundColor DarkGray

    $fallback = "C:\Temp\M5Discovery\$invokeScriptName"
    if (Test-ScriptIntegrity $fallback) {
        $DiscoveryScript = $fallback
        Write-Host "  (^_^)  Found valid copy at $fallback - using that." -ForegroundColor Green
        Write-Host "         TIP: Copy that file to this folder to avoid this check next time." -ForegroundColor DarkGray
    } else {
        Write-Host ""
        Write-Host "  (x_x)  No valid copy of $invokeScriptName found anywhere." -ForegroundColor Red
        Write-Host "         Expected location : $PSScriptRoot" -ForegroundColor DarkGray
        Write-Host "         Fallback checked  : C:\Temp\M5Discovery\" -ForegroundColor DarkGray
        Write-Host ""
        Write-Host "  Fix: Re-copy $invokeScriptName from C:\Temp\M5Discovery\ to this folder." -ForegroundColor Yellow
        exit 1
    }
}
    ...
    try {
        ...
        foreach ($vm in $vms) {
            ...
            try {
                ...
                if ($Cred) { $netWmiParams.Credential = $Cred }
                $adapters = Get-WmiObject @netWmiParams | Where-Object { $_.InstanceID -match $vm.Name }
                if ($adapters) {
...
function Test-WMIAccess {
    ...
    try {
        $p = @{ ComputerName=$Target; Class='Win32_ComputerSystem'; EA='Stop' }
...
function Get-WinRMStateViaWMI {
    param([string]$Target, [PSCredential]$Cred)
    try {
        $p = @{ ComputerName=$Target; Class='Win32_Service'; Filter="Name='WinRM'"; EA='Stop' }
        if ($Cred) { $p.Credential=$Cred }
        $svc = Get-WmiObject @p
...
function Enable-WinRMViaWMI {
    ...
    try {
        ...
        $pp = @{ ComputerName=$Target; Class='Win32_Process'; EA='Stop' }
        if ($Cred) { $pp.Credential=$Cred }
        (Get-WmiObject @pp).Create('powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Enable-PSRemoting -Force -SkipNetworkProfileCheck 2>&1|Out-Null"') | Out-Null
...
function Restore-WinRMState {
    ...
    try {
        ...
        (Get-WmiObject @pp).Create($cmd) | Out-Null
